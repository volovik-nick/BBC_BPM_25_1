...
chest_loot = {
    ...
    'снаряжение': {
        'рыцарь': [
            'железный топор (16 физического урона)',
...
secretroom = ['фонтан исцеления', 'меч в камне', 'котел', 'ведьма']
monsters_stats = {
...
clases = {
    'рыцарь': {
        ...
        'стартовый инвентарь': {
            'экипировка': ['старый топор (9 физического урона)','деревяный щит (2 защиты)','кожанный нагрудник (4 защиты)'],
            'зелья': []},
...
def moving(a):
    ...
    global game_field
    if a == '1':
        movements.append((movements[-1][0] - 1, movements[-1][1])) #up
    elif a == '2':
        movements.append((movements[-1][0] + 1, movements[-1][1])) #down
    elif a == '3':
        movements.append((movements[-1][0], movements[-1][1] + 1)) #right
    elif a == '4':
        movements.append((movements[-1][0], movements[-1][1] - 1)) #left
    player_view[movements[-1][0]][movements[-1][1]] = game_field[movements[-1][0]][movements[-1][1]] #приравниваем клетку из невидимой к видимой карте

#------------Функция определяющая дроп из сундука----------------------------
def open_chest(player_class):
    combination = random.choices(['1', '2'], weights=[0.65, 0.35], k=1)[0] #рандом выпадение зелья или артефакта
    # Получаем снаряжение (всегда одно, для класса игрока)
    result = random.choice(chest_loot['снаряжение'][player_class])
    player_class['стартовый инвентарь']['экипировка'].append(result) 
    if 'урона' in result:
        playing_class['стартовые характеристики']['damage'] = 12
    else:
        playing_class['стартовые характеристики']['armor'] += result[result.find('(') + 1] // 2
    # Получаем зелья и артефакты с учетом выпавшей комбинации
    if combination == '1':
        player_class['стартовый инвентарь']['зелья'].append(random.choices([p['item'] for p in chest_loot['зелья']], weights=[p['weight'] for p in chest_loot['зелья']], k=1))
    else:
        player_class['стартовый инвентарь']['экипировка'].append(random.choices([a['item'] for a in chest_loot['артефакты']], weights=[a['weight'] for a in chest_loot['артефакты']], k=1))
    return result

#------------Функция ловушки----------------------------
def trap(a):
    result = random.choice(a)
    if result == 'фонтан исцеления':
        a = input('Выпить из фонтана?\n ')
        if a == 'да':
            result_1 = random.choice(['-', '+'], [0.75, 0.25])
            if result_1 == '-':
                print('Максимальный запас здороья уменьшин(((')
                playing_class['стартовые характеристики']['hp'] -= 10
            else:
                print('Максимальный запас здороья увеличен)))')
                playing_class['стартовые характеристики']['hp'] += 10
        else:
            return 0
    elif result == 'котел':
        print('вы теряете все зелье')
        playing_class['стартовый инвентарь']['зелья'].clear()
    elif result == 'ведьма':
        print('вы получаете зелье')
        playing_class['стартовый инвентарь']['зелья'].append(random.choices([p['item'] for p in chest_loot['зелья']], weights=[p['weight'] for p in chest_loot['зелья']], k=1))
    elif result == 'меч в камне':
        a = input('Вы встречаете подозрительный камень с мечем в нем. попробовать вытащить его?\n ')
        if a == 'да':
            result_1 = random.choice(['-', '+'], [0.9, 0.1])
            if result_1 == '-' or 'рыцарь' not in playing_class:
                print('Вы не получаете ничего')
            else:
                print('Вы получаете святой меч')
                playing_class['стартовые характеристики']['damage'] = 17
                playing_class['стартовый инвентарь']['экипировка'].append('сявтой меч (урон 17 единиц)')
...
playing_class = {} #класс за который играют
portal = () #координаты портала
key = () #координаты ключа
...
#--------------Нахождение координат комнат ключа и портала--------------------------
a = 0
for e in game_field:
    for d in e:
        if 'портал' == d:
    ...
    a += 1
#--------------Игровой процесс--------------------------
for row in player_view: #отрисовка матрицы
        print(' | '.join(f'{cell:<17}' for cell in row))
while True:
    if movements[-1] == portal and key in movements: #если есть ключ и игрок в комнате портала 
        print('Победа')
        break
    if movements[-1] == key:
        playing_class['стартовый инвентарь']['экипировка'].append('ключ')
    print(' \n1.вверх\n2.вниз\n3.вправо\n4.влево\n ')
    while True: #перемещение, если игрок пытается выйти за границы то происходит ошибка
        choise = input('ваш выбор: ')
    ...
    for row in player_view: #отрисовка матрицы
        print(' | '.join(f'{cell:<17}' for cell in row))
    if game_field[movements[-1][0]][movements[-1][1]] == 'монстр': #бой с монстром
        monster_name = random.choice(list(monsters_stats.keys()))
        if not battle(playing_class, monster_name): #если фунция возвращает 0 то игрок умер
            print("Игра окончена.")
            break
        else:
            #замена состояния комнат
            game_field[movements[-1][0]][movements[-1][1]] = 'пусто' 
            player_view[movements[-1][0]][movements[-1][1]] = 'победа'
            for row in player_view:
                print(' | '.join(f'{cell:<17}' for cell in row))
    elif game_field[movements[-1][0]][movements[-1][1]] == 'сундук': #открвытие сундука
        open_chest()
        #замена состояния комнат
        game_field[movements[-1][0]][movements[-1][1]] = 'пусто'
        player_view[movements[-1][0]][movements[-1][1]] = 'сундук открыт'
    elif game_field[movements[-1][0]][movements[-1][1]] == 'секретная комната':
        trap(secretroom)
        #замена состояния комнат
        game_field[movements[-1][0]][movements[-1][1]] = 'пусто'
